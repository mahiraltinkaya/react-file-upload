...
import { ImageContainerProps } from "./index.d";

import "./index.scss";

const UploadContainer: React.FunctionComponent<ImageContainerProps> = ({
  ...
  const onDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    let fileList = Array.from(dt.files as FileList);
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    let fileList = Array.from(target.files as FileList);
  ...
  const setFileObject = (fileList: File[]) => {
    let total: number = 0;
  ...
  const setPreviewUrl = (file: any) => {
    return URL.createObjectURL(file);
  };

  return (
    <div className="file__container" {...others}>
